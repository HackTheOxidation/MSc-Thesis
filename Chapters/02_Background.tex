...
JVM bytecode is rather simple in the sense that every instruction is only one byte (except for floats and doubles, which are two bytes, or one word).
The bytecode in a ``class''-file is also grouped, first by class, then by method.
